...
- 求导会 **新建** 大量节点，但会通过构建期 **CSE（公共子表达式消除）** 复用结构相同的节点；例如重复出现的 `x * p`、`exp(x * p)` 会共享同一 DAG 节点。变量仍按 `Expr` 对象身份区分，`Expr::variable("x")` 调用两次会得到两个不同变量。
...
2. **子图共享与 CSE**：构建表达式时会对常量、复合算子和自定义算子按结构做 interning；DAG 中相同 `shared_ptr` 只分配一个 slot，多根（如 `loss` 与 `deriv_p`）共享公共子表达式。
